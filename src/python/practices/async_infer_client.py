...
# Create a condition variable to wait for the callback
...
# Send the asynchronous inference request
try:
    print("Sending asynchronous inference request...")
    triton_client.async_infer(model_name=model_name, inputs=inputs, outputs=outputs, callback=callback)
    print("Request sent. Waiting for response...")

    # Wait for the response or timeout
    with condition:
        condition.wait(timeout=3)  # Adjust the timeout as needed

    if not response_received:
        print("Timeout waiting for response.")
except utils.InferenceServerException as e:
    print(f"Failed to send asynchronous inference request: {e}")
except Exception as e:
    print(f"An unexpected error occurred: {e}")

print("Shutting down Triton client gracefully.")