...
class VehicleReID:
    """
    VehicleReID is a class that encapsulates the functionality for performing 
    vehicle re-identification using a model hosted on Triton Inference Server.
    ...
    def __init__(self, model_name, image_height=256, image_width=256, triton_url="localhost:8001"):
        """
        Initializes the VehicleReID class with the model name, image dimensions, and Triton server URL.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Vehicle Re-Identification using Triton Inference Server")
    ...
    parser.add_argument("--width", type=int, default=256, help="Width to resize the image to")
    args = parser.parse_args()

    # Create an instance of the VehicleReID class
    reid = VehicleReID(model_name=args.model_name, image_height=args.height, image_width=args.width)
